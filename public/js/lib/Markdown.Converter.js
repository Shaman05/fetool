...
(function() {
	...
	Markdown.Converter = function() {
		...
		function hashElement(wholeMatch, m1) {
			...
			blockText = "\n\n~K" + (g_html_blocks.push(blockText) - 1)
					+ "K\n\n";
		...
		function _EscapeSpecialCharsWithinTagAttributes(text) {
			...
			text = text.replace(regex, function(wholeMatch) {
				...
				tag = escapeCharacters(tag,
						wholeMatch.charAt(1) == "!" ? "\\`*_/" : "\\`*_"); // also escape slashes in comments to prevent autolinking there -- http://meta.stackoverflow.com/questions/95987
				return tag;
			});
			...
			if (g_list_level) {
				...
				text = text.replace(whole_list,
						function(wholeMatch, m1, m2, m3) {
							...
							var list_type = (m3.search(/[*+-]/g) > -1) ? "ul"
									: "ol";
							var result = _ProcessListItems(list, list_type);
		...
		function _ProcessListItems(list_str, list_type) {
			...
			list_str = list_str.replace(re, function(wholeMatch, m1, m2, m3) {
				...
				var contains_double_newline = ends_with_double_newline
						|| item.search(/\n{2,}/) > -1;

				if (contains_double_newline || last_item_had_a_double_newline) {
					item = _RunBlockGamut(_Outdent(item), /* doNotUnhash = */
							true);
				} else {
		...
		function _DoAutoLinks(text) {
			...
			var replacer = function(wholematch, m1) {
				return "<a href=\"" + m1 + "\">"
						+ pluginHooks.plainLinkText(m1) + "</a>";
			}
